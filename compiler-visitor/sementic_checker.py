...
from .ast import IdentDesc, IdentScope, EMPTY_STMT, EMPTY_IDENT, \
    AstNode, LiteralNode, IdentNode, TypeNode, BinOpNode, ExprNode, TypeConvertNode, CallNode, \
    VarsNode, VarNode, FuncNode, ParamNode, AssignNode, ReturnNode, IfNode, WhileNode, ForNode, StmtListNode
...
class SemanticChecker:
    ...
    def semantic_check(self, node: VarNode, scope: IdentScope) -> None:
        if node.type is not None:
            node.type.semantic_check(self, scope)
        node.var.semantic_check(self, scope)
        var = node.ident
        var_node: IdentNode = var if isinstance(var, AssignNode) else var
        node.var = type_convert(node.var, node.type.type if node.type is not None else None, node,
